...
export function ReviewPanel({ lines, totals, onQuantity, onSave, savedNotice, onCheckout }) {
  return (
    ...
    >
      <span className="text-[11px] tracking-[2px] text-[#3f4650] min-[1451px]:col-start-1">REVIEW</span>
      <h2 className="mt-5 mb-1 text-2xl leading-[1.1] font-extrabold min-[1451px]:col-start-1">Your security system</h2>
      <p className="m-0 leading-tight text-[#555c68] min-[1451px]:col-start-1">
        Review your personalized protection system designed to keep what matters most safe.
      </p>

      <div className="mt-3.5 border-t border-[#cfd7e1] min-[1451px]:col-start-1">
        {REVIEW_GROUPS.map((group) => {
          ...
          return (
            <section className="border-b border-[#cfd7e1] py-3 pb-2.5" key={group}>
              <h3 className="mb-2 text-xs font-medium text-[#a7b0bd] uppercase">{group}</h3>
              {groupLines.map((line) => (
      ...
      <div className="flex items-center justify-between border-b border-[#cfd7e1] py-3.5 min-[1451px]:col-start-1">
      ...
      <div className="mt-3.5 grid grid-cols-[92px_minmax(0,1fr)] items-center gap-3.5 min-[1451px]:col-start-2 min-[1451px]:row-start-1 min-[1451px]:row-span-4">
        ...
        <div className="col-span-full flex items-end justify-end gap-5">
      ...
      <p className="my-2 mt-3 text-center text-xs font-bold text-[#00a886] min-[1451px]:col-start-2">
        Congrats! You're saving {money(totals.savings)} on your security bundle!
      </p>
      <button
        className="inline-flex h-12 w-full items-center justify-center gap-2 rounded bg-[#5130e8] text-[17px] font-extrabold text-white min-[1451px]:col-start-2"
        type="button"
        onClick={onCheckout}
      >
        <CreditCard size={18} />
      ...
      <a className="mt-2.5 block text-center text-[13px] text-[#6c6f77] italic underline min-[1451px]:col-start-2" href="/" onClick={onSave}>
        Save my system for later
      </a>
      {savedNotice && <span className="mt-1.5 block text-center text-xs font-bold text-[#00a886] min-[1451px]:col-start-2">{savedNotice}</span>}
    </aside>
  )
}
  ...
  return (
    <div className="grid min-h-12 grid-cols-[42px_minmax(0,1fr)_auto] items-center gap-2.5 min-[761px]:grid-cols-[40px_minmax(0,1fr)_auto_58px]">
      ...
      <div className="col-start-2 justify-self-start min-[761px]:col-auto min-[761px]:justify-self-auto">
        <Stepper count={line.quantity} onMinus={() => onQuantity(line.key, -1)} onPlus={() => onQuantity(line.key, 1)} />
      </div>
      <span className="col-start-3 row-span-2 row-start-1 grid justify-items-end gap-0.5 self-center text-xs text-[#606773] min-[761px]:col-auto min-[761px]:row-auto">
        {line.compareAt !== line.price && <s className="text-[#69717c]">{money(line.compareAt * line.quantity)}</s>}
